...
# - physics consistency:
#     • uses correct masses (grams) and b_ij(T) values from parameters
#     • ensures F_crit ∝ (m_j − m_i) * b_ij / [k_B T (1 + f_j)]
# overall:
...
def test_i_picks_lightest_present(monkeypatch, base_geo):
    ...
    # No H present => lightest *present* should be C
    def fake_counts(_p):
        return dict(H=0.0, C=1.0, N=2.0, O=3.0, S=4.0)
    monkeypatch.setattr(FractionationPhysics, "atomic_counts_from_X", staticmethod(fake_counts))

    i, j, f = FractionationPhysics.choose_light_and_heavy_major(p, RXUV, T, m_p, allow_dynamic_light_major=True)
    assert i == "C"
    # j should be the most abundant heavier-than-C => O (since f_O>f_N here)
    assert j == "O"

def test_j_by_abundance_then_fcrit_tiebreak(monkeypatch, base_geo):
    # i = H present, two heavies (O, C) with nearly equal f; tie-break on smallest Fcrit
    # Fcrit ∝ (m_j - m_i)*b_ij / (T*(1+f_j))  (g & constants same for both)
    # Make b(H,O) < b(H,C) so O gets smaller Fcrit and is chosen.
    p = Parameters(b_map={("H","O"): 4.8e17, ("H","C"): 6.0e17})
    RXUV, m_p, T = base_geo

    def fake_counts(_p):
        # Ni sets f via f_s = N_s/N_i. Keep O and C very close so tol_major matters.
        return dict(H=10.0, C=9.9, N=0.0, O=10.0, S=0.0)
    monkeypatch.setattr(FractionationPhysics, "atomic_counts_from_X", staticmethod(fake_counts))

    # Use a relatively generous tol_major so both O & C are in the near-top set
    i, j, f = FractionationPhysics.choose_light_and_heavy_major(
        p, RXUV, T, m_p, allow_dynamic_light_major=True, tol_major=0.02
    )
    assert i == "H"
    assert j == "O"   # chosen by smaller Fcrit among the near-top pair
